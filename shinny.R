...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      selectInput("countryInput", "PINNs Solution for:",
                  choices = c("Simple ODE (y'-y=0)", "Burgers Equation" )),
      sliderInput("lay","Number of Layers",min=2,max = 6,value = 5,step=1),
      sliderInput("neu","Number of Neurons",min=10,max = 60,value = 50,step=1),
      sliderInput("it","Iteration",min=2000,max = 20000,value = 2000, step=2000),
...
server <- function(input, output, session) {
  observeEvent(input$start_pb, {
    ...
    message = function(m) {
      shinyjs::html(id = "plot1", html = m$message, add = FALSE)
    })
  })
  observeEvent(input$start_pb, {
    withCallingHandlers({
    ...
    message = function(m) {
      shinyjs::html(id = "plot2", html = m$message, add = FALSE)
    })
  })
  observeEvent(input$start_pb, {
    withCallingHandlers({
    ...
    message = function(m) {
      shinyjs::html(id = "plot3", html = m$message, add = FALSE)
    })
  })
  observeEvent(input$start_pb, {
    withCallingHandlers({
    ...
    message = function(m) {
      shinyjs::html(id = "plot4", html = m$message, add = FALSE)
    })
  })
  observeEvent(input$start_pb, {
    withCallingHandlers({
    ...
    message = function(m) {
      shinyjs::html(id = "plot5", html = m$message, add = FALSE)
    })
  })
  ...
  df<-reactive({input$act
    ...
    layers <-c(1, rep(isolate(input$neu), isolate(input$lay)), 1)
  ...
  output$plot4<-renderPlot({input$act
    layers <-c(1, rep(isolate(input$neu), isolate(input$lay)), 1)
  ...
  output$plot6<-renderPlot({input$act
    layers <-c(1, rep(isolate(input$neu), isolate(input$lay)), 1)
